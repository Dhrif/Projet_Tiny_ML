...
/* Includes ---------------------------------------------------------------- */
#include <Classification_Composants_inferencing.h>
...
// Print all class probabilities (Edge Impulse default debug output)
...
// ------------------------------------------------------------
// ADDITIONAL SECTION: build a structured JSON line for Node-RED
// This makes parsing much easier on the server side.
// ------------------------------------------------------------

// Variable storing the index of the most probable class
int predicted_class_index = -1;

// Variable storing the highest probability detected
float highest_probability = -1.0f;

// Search for the class with the highest probability
for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {

    float current_probability = result.classification[i].value;
...
// ------------------------------------------------------------
// Send a JSON message on a single line
// The prefix "EI:" is used so Node-RED can easily filter it
// ------------------------------------------------------------

ei_printf("EI:{\"best\":\"%s\",\"probability\":%.5f",
          ei_classifier_inferencing_categories[predicted_class_index],
          highest_probability);

// Add probability for every class in the JSON object
for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
